...
const ProductCard: FC<ProductCardProps> = (props: ProductCardProps) => {
    ...
    let likeProduct = async () => {
        if (LikesProduct.length === 0) {
            await axios.post('http://localhost:3001/Likes', { LikesProductId: props.Id, user: User.userName });
        } else if (LikesProduct[props.Id]?.LikesProductId !== props.Id) {
            await axios.post('http://localhost:3001/Likes', { LikesProductId: props.Id, user: User.userName });
        } else {
            await axios.delete('http://localhost:3001/Likes/' + LikesProduct[props.Id].id);
        }
