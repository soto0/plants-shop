...
import { addLike, deleteLike } from '../../../api/LikeToggle';
...
const ProductCard: FC<ProductCardProps> = (props: ProductCardProps) => {
    ...
    let likeProduct = async () => {
        if (User.length === 0) {
            ...
            if (!props.LikesProduct[0] && User.length !== 0) {
                addLike(User.userName);
            } else {
                deleteLike(props.LikesProduct[0].id);
            };
    ...
    let addToBasket = () => {
        if (User.length === 0) {
            ...
            if(!props.BasketToggleProduct[0] && User.length !== 0) {
                addProductToBasket(props.Id, props.Title, props.Price, User.userName);
            } else {
                deleteProductFromBasket(props.Id);
            };
