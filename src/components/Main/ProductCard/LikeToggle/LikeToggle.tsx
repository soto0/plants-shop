...
const LikeToggle: FC<LikeToggleProps> = (props: LikeToggleProps) => {
    let likeProduct = async () => {
        if (props.User.length === 0) {
            props.SetPromptActive(true);
        } else {
            if (!props.LikesProduct[0] || props.LikesProduct[0].likesProductId !== props.Id) {
                addLike(props.Id, props.User.userName);
            } else {
                deleteLike(props.LikesProduct[0].id);
            };
            
            props.GetLikesProduct(props.User.userName);
        };
    };
